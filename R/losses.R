...
euclidean_distance_loss <- function(x, z_candidates) {
  as.vector(sqrt(rowSums((
    as.list(x) - z_candidates
  ) ^ 2)))
}
...
gower_distance_loss <-
  function(x,
    ...
    if (!is.null(categorical_variables_indices)) {
      distance <-
        rowSums(xx[, categorical_variables_indices, drop = FALSE] != zz[, categorical_variables_indices, drop =
                                                                          FALSE])
...
plausiblity_knn_loss <-
  function(z,
           ...
           k = 5) {
    dist_matrix <-
      gower_distance_loss(z,
                          background_data,
                          data_range,
                          categorical_variables_indices,
                          1)
    return(apply(dist_matrix, 1, function(x)
      mean(sort(x)[1:k])))
  }


kovalev_counterfactual_loss <-
  function(explainer,
           x_mean_value,
           z_candidates,
           xz_distances,
