#' @title Kovalev method
#' @description Kovalev method is a model-agnostic counterfactual explanation approach for survival models. It is based on the Particle Swarm Optimization algorithm.
#'
...
#' @param r The target difference between the mean time to survival of the counterfactual and the original observation.
#' @param num_iter Number of iterations in the PSO algorithm.
#' @param num_particles Number of particles in the PSO algorithm.
#' @param verbose Integer value indicating how often the loss should be printed.
...
#' @param tol_iter Integer value indicating the number of iterations without progress after which the search should be stopped.
...
kovalev_method <-
  function(explainer,
    ...
    z_candidates <-
      data.frame(scale(z_candidates, center = mu, scale = sigma))
    ...
    # calculate losses
    losses <- kovalev_counterfactual_loss(
      explainer,
      mean_value,
    ...
    validity_mask <- r - theta * (mean_time_to_survival(explainer, rescale_to_original(z_candidates, mu, sigma))
                                  - mean_value) <= 0
    ...
    if (sum(validity_mask) == 0) {
      ...
      z_closest <-
        z_candidates[validity_mask,][which.min(losses[validity_mask]),]
    ...
    set.seed(seed)
    particles <-
      data.frame(apply(data_range, 1, function(var_range)
        runif(num_particles, var_range[1], var_range[2])))
    particles <-
      restriction_procedure(x,
                            particles,
    ...
    velocities <-
      matrix(0, nrow = num_particles, ncol = ncol(particles))
    best_positions <- particles

    best_losses <- kovalev_counterfactual_loss(
      explainer,
      mean_value,
    ...
    for (i in 1:num_iter) {
      ...
      particles <- restriction_procedure(x,
                                         particles,
                                         euclidean_distance_loss(x, particles),
                                         radius_closest,
                                         data_range)
      ...
      best_positions[losses < best_losses,] <-
        particles[losses < best_losses,]
      best_losses[losses < best_losses] <-
        losses[losses < best_losses]

      history <-
        rbind(history, data.frame(generation = i, loss = best_losses))
      ...
      if (min(best_losses) < best_loss) {
        ...
        if (invalid_counter >= tol_iter) {
          warning(paste(
            "Seach stopped due to no progress in the last",
            tol_iter,
            "iterations."
          ))
          break
    ...
    final_preds <-
      predict(explainer, rescale_to_original(best_positions, mu, sigma))
    ...
    class(result) <-
      c("counterfactual_explanations",
        "Kovalev_method_counterfactuals")
    return(result)
  }


restriction_procedure <-
  function(x,
           z_candidates,
           xz_distances,
           radius_closest,
           data_range) {
    z_candidates <-
      as.list(x) + pmin(radius_closest, xz_distances) * (z_candidates - as.list(x)) / xz_distances
