...
plot_predictions <-
  function(preds,
    ...
    preds_plot <-
      prepare_predictions_to_plot(preds, times, preds_labels, alpha, linewidth)

    with(preds_plot,
         {
           ggplot(preds_plot, aes(
             x = time,
             y = fun,
             group = id,
             color = cluster
...
#' @param digits a numeric value indicating the number of digits to round the variable values
#'
#' @export
plot_parallel_coordinates <-
  function(counterfactual_explanations,
           filtered_examples = NULL,
           variables = NULL,
           digits = 2) {
    if (is.null(variables)) {
      variables <-
        1:ncol(counterfactual_explanations$counterfactual_examples)
    } else if (is.character(variables)) {
      variables <-
        match(variables,
              colnames(counterfactual_explanations$counterfactual_examples))
    ...
    if (is.null(filtered_examples)) {
      ...
      filtered_examples_validity <-
        counterfactual_explanations$objective_values$validity
    } else if (!all(
      colnames(counterfactual_explanations$counterfactual_examples) == colnames(filtered_examples[, 1:ncol(counterfactual_explanations$counterfactual_examples)])
    )) {
      stop(
        "filtered_examples must have the same columns as counterfactual_explanations$counterfactual_examples"
      )
    } else {
      filtered_examples_validity <- filtered_examples$validity
      filtered_examples <-
        filtered_examples[, 1:ncol(counterfactual_explanations$counterfactual_examples)]
    }
    numeric_mask <-
      sapply(counterfactual_explanations$original_observation,
             is.numeric)
    whole_population <-
      rbind(
    ...
    categorical_variables_orderings <-
      lapply(counterfactual_explanations$counterfactual_examples[, !numeric_mask, drop =
                                                                   FALSE],
             function(x) {
    ...
    whole_population[, !numeric_mask] <-
      lapply(whole_population[, !numeric_mask, drop =
                                FALSE],
             function(x)
               as.numeric(as.factor(x)))

    filtered_examples[, !numeric_mask] <-
      lapply(filtered_examples[, !numeric_mask, drop =
                                 FALSE],
             function(x)
               as.numeric(as.factor(x)))

    original_obs <- counterfactual_explanations$original_observation
    original_obs[!numeric_mask] <-
      lapply(original_obs[, !numeric_mask, drop =
                            FALSE],
             function(x)
               as.numeric(as.factor(x)))
    ...
    filtered_examples <-
      t((t(filtered_examples) - mins) / (maxs - mins))
    ...
    plot_df$type <-
      c("original_observation", rep("counterfactual", nrow(filtered_examples)))
    ...
    if (!all(numeric_mask)) {
      mins[!numeric_mask] <-
        sapply(categorical_variables_orderings, function(x)
          names(x)[which.min(x)])
      maxs[!numeric_mask] <-
        sapply(categorical_variables_orderings, function(x)
          names(x)[which.max(x)])
    ...
    text_df <- data.frame(
      variable = rep(1:length(variables), 2),
      value = rep(c(-0.05, 1.05), each = length(variables)),
      label = c(mins[variables], maxs[variables])
    )

    long_plot_df <-
      reshape2::melt(plot_df, id.vars = c("validity", "type", "id"))
    ...
    with(long_plot_df,
         {
           ggplot(long_plot_df[long_plot_df$type == "counterfactual", ],
             ...
             geom_line(
               data = long_plot_df[long_plot_df$type == "original_observation", ],
             ...
             geom_text(
               data = text_df,
               inherit.aes = FALSE,
               aes(x = variable, y = value, label = label),
               size = 4
             )
         })
...
#' @title Plot frequency of variable changes in counterfactual explanations
#'
#' @param counterfactual_explanations a `counterfactual_explanations` object with counterfactual examples, objective values and original observation
#' @param filtered_examples a data frame with counterfactual examples. If NULL, all counterfactual examples from `counterfactual_explanations` will be used
#' @param variables a numeric vector of column indices representing variables to plot. If NULL, all variables will be on the plot
#' @param max_variables a numeric value indicating the maximum number of variables to plot
#'
#' @export
plot_changes_frequency <-
  function(counterfactual_explanations,
           filtered_examples = NULL,
           variables = NULL,
           max_variables = Inf) {
    ...
    plot_df <-
      colMeans(data.frame(
        as.list(counterfactual_explanations$original_observation) != filtered_examples
      ))
    plot_df <- plot_df[variables]
    ...
    long_plot_df <- long_plot_df[order(-long_plot_df$value),]
    long_plot_df <-
      long_plot_df[1:min(nrow(long_plot_df), max_variables),]

    with(long_plot_df,
         {
           ggplot(long_plot_df, aes(
...
#'
#' @param counterfactual_explanations a `counterfactual_explanations` object with counterfactual examples, objective values and original observation
#' @param filtered_predictions a matrix of filtered counterfactual predictions. If NULL, all counterfactual predictions from `counterfactual_explanations` will be used
#' @param filtered_examples a data frame with counterfactual examples. If NULL, all counterfactual examples from `counterfactual_explanations` will be used
#' @param function_type a character value indicating the type of function. Either "survival" or "chf". If NULL, the function type will be inferred from the `counterfactual_explanations` object
#'
#' @export
plot_counterfactual_predictions <-
  function(counterfactual_explanations,
    ...
    if (is.null(filtered_examples)) {
      filtered_examples <-
        counterfactual_explanations$counterfactual_examples
      filtered_examples_objective_values <-
        counterfactual_explanations$objective_values
    } else {
      filtered_examples_objective_values <-
        filtered_examples[, (ncol(counterfactual_explanations$counterfactual_examples) +
                               1):ncol(filtered_examples)]
      filtered_examples <-
        filtered_examples[, 1:ncol(counterfactual_explanations$counterfactual_examples)]
    }
    ...
    original_prediction <-
      counterfactual_explanations$original_prediction[1,]
    target_prediction_band <-
      counterfactual_explanations$target_prediction_band
    numeric_mask <-
      sapply(counterfactual_explanations$original_observation,
             is.numeric)
    ...
    if (is.null(function_type)) {
      ...
      if (counterfactuals_type == "multiobjective_counterfactuals") {
        filtered_predictions <-
          translate_survival_to_cumulative_hazard(filtered_predictions)
        original_prediction <-
          translate_survival_to_cumulative_hazard(original_prediction)
      } else if (counterfactuals_type == "treebased_counterfactuals") {
        filtered_predictions <-
          translate_cumulative_hazard_to_survival(filtered_predictions)
        original_prediction <-
          translate_cumulative_hazard_to_survival(original_prediction)
      }
      target_prediction_band <-
        translate_target_prediction_band(target_prediction_band)
    }
    ...
    colnames(filtered_predictions) <-
      counterfactual_explanations$times
      ...
      fig <- fig %>% add_trace(
        ...
        hovertemplate =
          paste0(
            ...
            "<b>Loss values:</b> <br>",
            "<i>Validity:</i> ",
            round(loss_values["validity"], 3),
            "<br>",
            "<i>Similarity:</i> ",
            round(loss_values["similarity"], 3),
            "<br>",
            "<i>Plausibility:</i> ",
            round(loss_values["plausibility"], 3),
            "<br>",
            "<i>Sparsity:</i> ",
            loss_values["sparsity"],
            "<br>",
            "<b>Variable values:</b><br>",
            var_values_str,
...
prepare_predictions_to_plot <-
  function(preds,
    ...
    preds_plot <-
      reshape2::melt(
        preds_plot,
        id.vars = c("id", "cluster", "alpha", "linewidth"),
        value.name = "fun",
        variable.name = "time"
      )
