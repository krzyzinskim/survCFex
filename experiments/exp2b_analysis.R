...
ggsave("experiments/plots/exp2b_objective_values.pdf", dpi=500, width=10, height=8)
...
p1 <- ggplot(time_df, aes(x = factor(pop_size), y = time)) +
  geom_boxplot(staplewidth = 0.5,
               outlier.shape = 1,
               outlier.size = 0.5,
               fill = "ghostwhite",
...
time_df %>%
  ...
  theme_bw() +
  theme(legend.position = "bottom")
ggsave("experiments/plots/exp2b_time_per_iteration.pdf", dpi=500, width=5, height=3.5)
