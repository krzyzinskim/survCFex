...
nrow(objective_values) / length(experiment5_results$tb_results) == 10
...
ggsave("experiments/plots/exp2b_bis_objective_values.pdf", dpi=500, width=10, height=8)
...
nrow(param_combinations) == length(experiment5_results$tb_time)
...
ggplot(time_df, aes(x = factor(paths_per_tree), y = time, color = factor(paths_per_counterfactual))) +
  geom_boxplot(staplewidth = 0.5,
               outlier.shape = 1,
               outlier.size = 0.5,
               fill = "ghostwhite",
