...
sapply(experiment2_results$moc_results, function(x) nrow(x$objective_values))

n_counterfactuals <- 10
n_parameter_sets <- nrow(experiment2_results$param_combinations)
n_observations <- 20

objective_values <- do.call("rbind", lapply(experiment2_results$moc_results,
                        function(res){
...
nrow(objective_values) / length(experiment2_results$moc_results) == 10
nrow(objective_values) / (n_parameter_sets * n_observations) == 10

param_combinations <- experiment2_results$param_combinations[rep(1:30, each=n_counterfactuals * n_observations), ]
...
# for big populations, there are visible differences
ggplot(res_df_long[res_df_long$pop_size %in% c(100, 200), ], aes(x = factor(pop_size), y = value, color = init_method)) +
  geom_boxplot(staplewidth = 0.5,
               outlier.shape = 1,
               outlier.size = 0.5,
               fill = "ghostwhite",
  ...
  theme_bw() +
  theme(legend.position = "bottom")
ggsave("experiments/plots/exp2a_objective_values.pdf", dpi=500, width=10, height=8)
...
ggplot(n_valid, aes(x = factor(pop_size), y = mean_valid, fill = init_method)) +
  ...
  theme_bw() +
  theme(legend.position = "bottom")
ggsave("experiments/plots/exp2_validity.pdf", dpi=500, width=10, height=4)




# TIME
n_iterations <- sapply(experiment2_results$moc_results, function(x) max(x$history$generation))
param_combinations <- experiment2_results$param_combinations[rep(1:30, each=n_observations), ]
nrow(param_combinations) == length(n_iterations)
nrow(param_combinations) == length(experiment2_results$moc_time)

time_df <- cbind(param_combinations, n_iterations = n_iterations, time=experiment2_results$moc_time)
...
ggsave("experiments/plots/exp2_time_iterations.pdf", dpi=500, width=10, height=6)
...
ggsave("experiments/plots/exp2_time_per_iteration.pdf", dpi=500, width=5, height=3.5)
...
m1 <- lm(time_df$time_per_iteration ~ time_df$pop_size - 1)
summary(m1)
coef(m1)
...
indices <- which(param_combinations$valid_threshold %in% c("0", "median", "none"))
